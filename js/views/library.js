import {el, esc, r0, rCal} from '../helpers.js';
...
export function openLibrary(){
  ...
  const distinct=()=>{const seen=new Set(),out=[];for(let i=state.entries.length-1;i>=0;i--){const e=state.entries[i],k=(e.name||'').toLowerCase();if(!e.name||seen.has(k))continue;seen.add(k);out.push({name:e.name,sub:`${e.servingLabel||(e.grams?r0(e.grams)+' g':'')} · ${rCal(e.calories)} kcal`,entry:e});}return out;};
  ...
  function showSaved(){
    ...
    body.innerHTML=lib.map(l=>{const sub=l.perServing?`${rCal(l.perServing.calories)} kcal per ${esc(l.perServing.portion)}`:`${rCal(l.per_100g.calories)} kcal/100g`;return rowHtml(l.name,sub,`data-lib="${l.id}"`);}).join('');
